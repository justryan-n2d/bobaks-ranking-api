...
function groupSnapshots(snapshots: SnapshotRow): never {
...
export function getNextRetentionDate(now = new Date(), retentionDays = DEFAULT_RETENTION_DAYS): Date {
  const cutoff = getRetentionCutoff(now, retentionDays);
  return addUtcDays(cutoff, 1);
}
